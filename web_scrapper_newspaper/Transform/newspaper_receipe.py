...
def _read_data(filename):
    ...
    return pd.read_csv(filename, encoding='windows-1252')
...
def _fill_missing_titles(df):
    ...
    missing_titles = (
        ...
        .applymap(lambda title_cap: title_cap.capitalize())
    )
...
def _save_data(df, filename):
    logger.info('Saving dataframe in CSV')
    name_csv ="clean_{}.csv".format(filename)    
    df.to_csv(name_csv, index=True)
    logger.info('Name CSV file: {}'.format(name_csv))
